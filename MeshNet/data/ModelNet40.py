...
class ModelNet40(data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        impurity_label = data['impurity_label']
        ...
        impurity_label = torch.from_numpy(impurity_label).float()

        # reorganize
        face = face.permute(1, 0).contiguous()
        impurity_label = impurity_label.permute(1, 0).contiguous()
        centers, corners, normals = face[:3], face[3:12], face[12:]
        corners = corners - torch.cat([centers, centers, centers], 0)

        return centers, corners, normals, neighbor_index, targets, impurity_label
