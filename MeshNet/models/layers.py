...
class FaceRotateConvolutionWithImpurity(nn.Module):

    def __init__(self):
        super(FaceRotateConvolutionWithImpurity, self).__init__()
        self.rotate_mlp = nn.Sequential(
            nn.Conv1d(7, 32, 1),
            nn.BatchNorm1d(32),
            nn.ReLU(),
    ...
    def forward(self, corners):
        ...
        fea = (self.rotate_mlp(torch.cat([corners[:, :6], corners[:, -1:]], 1)) +
               self.rotate_mlp(torch.cat([corners[:, 3:9], corners[:, -1:]], 1)) +
               self.rotate_mlp(torch.cat([corners[:, 6:9], corners[:, :3], corners[:, -1:]], 1))) / 3
...
class FaceKernelCorrelation(nn.Module):
    ...
    def forward(self, normals, neighbor_index, impurity_label):
        ...
        #impurity_label = impurity_label.unsqueeze(2).expand(-1, -1, self.num_kernel, -1).unsqueeze(4)

        fea = torch.cat([center, neighbor], 4)
        #fea = torch.cat([fea, impurity_label], 1)
...
class StructuralDescriptor(nn.Module):
    ...
    def forward(self, corners, normals, neighbor_index, impurity_label):
        structural_fea1 = self.FRC(corners)
        structural_fea2 = self.FKC(normals, neighbor_index, impurity_label)

        return self.structural_mlp(torch.cat([structural_fea1, structural_fea2, normals, impurity_label], 1))
