...
def move_all_objects(t_x, t_y, t_z):
    bpy.ops.object.select_all(action='SELECT')
    bpy.ops.transform.translate(value=(t_x, t_y, t_z), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)


def scale_all_objects(scale):
    bpy.ops.object.select_all(action='SELECT')
...
for j in range(0, 100):
    ...
    for i in range(1, 19):
        ...
        while True:
            if (time.time() - start_time) > 5 or total_blobs > 40:
                break
    ...
    for i in range(10):
        ...
        t_x = np.random.uniform(-10, 10)
        t_y = np.random.uniform(-10, 10)
        t_z = np.random.uniform(-10, 10)
        ...
        f.close()
        target_file = os.path.join(directory + "\\data_8", "automated_with_diamond_" + str(file_count) + ".obj")
        bpy.ops.object.select_all(action='SELECT')
        bpy.ops.export_scene.obj(filepath=target_file, use_selection=True)
        bpy.ops.object.select_all(action='DESELECT')
        diamond.select_set(True)
        diamond_file = os.path.join(directory + "\\data_8", "diamond_" + str(file_count) + ".obj")
        bpy.ops.export_scene.obj(filepath=diamond_file, use_selection=True)
        bpy.ops.object.select_all(action='SELECT')
        diamond.select_set(False)
        target_file = os.path.join(directory + "\\data_8", "automated_without_diamond_" + str(file_count) + ".obj")
        bpy.ops.export_scene.obj(filepath=target_file, use_selection=True)
        bpy.ops.object.select_all(action='DESELECT')
        for cube in cubes:
            cube.select_set(True)
        cubes_file = os.path.join(directory + "\\data_8", "rocks_" + str(file_count) + ".obj")
        bpy.ops.export_scene.obj(filepath=cubes_file, use_selection=True)
        bpy.ops.object.select_all(action='DESELECT')
        outer_sphere.select_set(True)
        outer_sphere_file = os.path.join(directory + "\\data_8", "outer_sphere_" + str(file_count) + ".obj")
