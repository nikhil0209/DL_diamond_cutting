...
def unet_loss(output, center_points, target_scale_and_rotation, alpha=0.5):
    #binary cross entropy loss for center predictions
    #target vector
    center_probs = output[:,:1,:,:,:]
    target = torch.zeros(center_probs.squeeze(1).shape).to(center_probs)
    for i, x in enumerate(center_points):
        target[i,x[0],x[1],x[2]]=1
    center_probs_lin = center_probs.reshape(center_probs.shape[0],-1)
    ...
    pred_rot_scale = output[:,1:,:,:,:]
    rot_scale_loss = 0
    for i, x in enumerate(center_points):
        rot_scale_loss+=torch.nn.L1Loss()(pred_rot_scale[i,:,x[0],x[1],x[2]], target_scale_and_rotation[i])
    #L1 loss for rotation and scale predictions
    rot_scale_loss/= pred_rot_scale.shape[0]
    return alpha * center_loss + (1-alpha) * rot_scale_loss

def get_output_from_prediction_new(output, pitch, radius):
    center_probs = output[:,:1,:,:,:]
    #only works for batch=1
    center_probs_lin = center_probs.reshape(center_probs.shape[0],-1)
    center_probs_lin = nn.functional.softmax(center_probs_lin,-1)
    ...
    #convert center_idx to a coordinate using center of that voxel
    center = (center_idx-radius)*pitch
    pred_rot_scale = center_probs = output[:,1:,center_idx[0],center_idx[1],center_idx[2]]
    outputs = torch.cat((center.unsqueeze(0),pred_rot_scale),dim=-1)
    return outputs

def yolo_loss(prediction, targets, center_points, unit_diamond_vertices, alpha = 1):
    # predictions shape = [batch, channels, h, w, b]
    # channels = [x,y,z,r_x,r_y,r_z,scale,confidence]
    # flatten predictions
    bs, ch, h, w, b = prediction.shape
    output = prediction.reshape(bs,ch,-1).transpose(1,2)
    weights = output[:,:,-1].reshape(-1)
    boxes = output[:,:,:-1].reshape(-1,ch-1)
    targets = targets.unsqueeze(1).expand(-1,32768,-1).reshape(-1,ch-1)

    box_loss = point_wise_L1_loss(boxes, targets, unit_diamond_vertices)
    confidence_loss = confidence_loss(prediction[:,-1:,:,:,:], center_points)

    return box_loss + alpha * confidence_loss

def point_wise_weighted_L1_loss(boxes, targets, weights, unit_diamond_vertices):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    loss = torch.tensor(0.0).to(device)
    for i in range(boxes.shape[0]):
        r_target = torch.from_numpy(R.from_euler('xyz', [[targets[i, 3], targets[i, 4], targets[i, 5]]]).as_matrix()).float().to(device)
        target_vertices = torch.mul(targets[i, -1], (torch.matmul(r_target, unit_diamond_vertices))) + torch.reshape(targets[i, 0:3], (3, 1))
        r_output = torch.from_numpy(R.from_euler('xyz', [[boxes[i, 3], boxes[i, 4], boxes[i, 5]]]).as_matrix()).float().to(device)
        output_vertices = torch.mul(boxes[i, -1], (torch.matmul(r_output, unit_diamond_vertices))) + torch.reshape(boxes[i, 0:3], (3, 1))
        loss += nn.L1Loss(reduction='mean')(torch.transpose(target_vertices, 0, 1), torch.transpose(output_vertices, 0, 1))
    loss /= boxes.shape[0]
    return loss

def confidence_loss(center_probs, center_points):
    #binary cross entropy loss for center predictions
    #target vector
    target = torch.zeros(center_probs.squeeze(1).shape).to(center_probs)
    for i, x in enumerate(center_points):
